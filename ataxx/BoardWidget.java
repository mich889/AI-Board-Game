...
class BoardWidget extends Pad  {
    ...
    static final int PIECE_CONSTANT = (int)(0.5*SQDIM - PIECE_RADIUS);

    static final int BLOCK_CONSTANT = (int)(0.5*SQDIM - (0.5)*BLOCK_WIDTH);
    ...
    public synchronized void paintComponent(Graphics2D g) {
        g.setBackground(BLANK_COLOR);
        g.setColor(Color.black);
        for (int i = 0; i <= _dim; i += SQDIM){
            for (int j = 0; j <= _dim; j+= SQDIM){
                g.drawRect(i,j,SQDIM,SQDIM);
            }
        }
        for (int i = 0; i < SIDE; i++){
            for (int j = 0; j < SIDE; j++){
                PieceColor currPiece = _model.get((char)('a'+i), (char)('1'+j));
                if (currPiece == RED){
                    g.setColor(RED_COLOR);
                    g.fillOval(i*SQDIM + PIECE_CONSTANT, (_dim-SQDIM-SQDIM*j) + PIECE_CONSTANT, PIECE_RADIUS*2, PIECE_RADIUS*2);
                } else if (currPiece == BLUE){
                    g.setColor(BLUE_COLOR);
                    g.fillOval(i*SQDIM + PIECE_CONSTANT, (_dim-SQDIM-SQDIM*j) + PIECE_CONSTANT, PIECE_RADIUS*2, PIECE_RADIUS*2);
                } else if (currPiece == BLOCKED){
                    drawBlock(g,i,j);
                }
            }
        }
    ...
    void drawBlock(Graphics2D g, int cx, int cy) {
        ...
        g.fill3DRect(cx*SQDIM+BLOCK_CONSTANT,cy*SQDIM+BLOCK_CONSTANT,BLOCK_WIDTH,BLOCK_WIDTH,true);
    }
    ...
    private void handleClick(String unused, MouseEvent where) {
        ...
        if (where.getButton() == MouseEvent.BUTTON1) {
            ...
            if (mouseCol >= 'a' && mouseCol <= 'g'
                ...
                } else if (_selectedCol != 0 && _model.get(_selectedCol,_selectedRow) == _model.whoseMove()){
                    if (_model.legalMove(_selectedCol,_selectedRow,mouseCol,mouseRow)) {
